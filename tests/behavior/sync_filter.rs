...
fn init() -> anyhow::Result<(SyncRootId, Connection<Arc<MemFilter>>)> {
...
fn test_list_folders() {
    let output = powershell_script::run(&format!("Get-ChildItem {ROOT_PATH} -Recurse -Name"))
        .expect("run script");
    assert_eq!(
        "dir1\r\n\
        test1.txt\r\n\
...
fn test_read_file() {
    for relative in ["test1.txt", "dir1\\test2.txt"] {
        let path = format!("{ROOT_PATH}\\{relative}");
        let output =
